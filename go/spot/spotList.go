...
// ### 12 开启OB抵扣 Enable OB Deduct
func ObDeduct(jsonParams string) interface{} {
	caseUrl := "/obDeduct/enable"
	requestUrl := config.BASE_URL + caseUrl
	fmt.Println("requestUrl:", requestUrl)
	response := utils.PrivatePost(requestUrl, jsonParams)
	return response
...
// ### 13 查看OB抵扣状态 Query OB Deduct Status
func QueryObDeduct(jsonParams string) interface{} {
	caseUrl := "/obDeduct/enable"
	requestUrl := config.BASE_URL + caseUrl
	fmt.Println("requestUrl:", requestUrl)
	response := utils.PrivateGet(requestUrl, jsonParams)
...
// ### 12 获取小额资产可兑换列表 Get Assets That Can Be Converted Into OB
