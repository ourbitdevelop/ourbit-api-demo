...
class TOOL(object):
    ...
    def _get_server_time(self):
        return requests.request('get', 'https://api.ourbit.com/api/v3/time').json()['serverTime']
    ...
    def _sign_v3(self, req_time, sign_params=None):
        ...
        sign = hmac.new(self.ourbit_secret.encode('utf-8'), to_sign.encode('utf-8'), hashlib.sha256).hexdigest()
        return sign
    ...
    def sign_request(self, method, url, params=None):
        ...
        headers = {
            'x-ourbit-apikey': self.ourbit_key,
...
# Market Data
class ourbit_market(TOOL):

    def __init__(self, ourbit_hosts):
        self.api = '/api/v3'
        self.hosts = ourbit_hosts
        self.method = 'GET'
...
# Spot Trade
class ourbit_trade(TOOL):

    def __init__(self, ourbit_hosts, ourbit_key, ourbit_secret):
        self.api = '/api/v3'
        self.hosts = ourbit_hosts
        self.ourbit_key = ourbit_key
        self.ourbit_secret = ourbit_secret
    ...
    def post_batchorders(self, params):
        ...
        params['signature'] = hmac.new(key=self.ourbit_secret.encode('utf-8'), msg=query_str.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
        headers = {
            'x-ourbit-apikey': self.ourbit_key,
            'Content-Type': 'application/json',
        }
    ...
    def post_obDeDuct(self, params):
        """Enable OB DeDuct"""
        method = 'POST'
        url = '{}{}'.format(self.api, '/obDeduct/enable')
        response = self.sign_request(method, url, params=params)
        return response.json()

    def get_obDeDuct(self):
        """OB DeDuct status"""
        method = 'POST'
        url = '{}{}'.format(self.api, '/obDeduct/enable')
        response = self.sign_request(method, url)
